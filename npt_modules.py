...
class MAB(nn.Module):
    ...
    def __init__(
        ...
        * `F_A` is dimension of the embedding at input
        ...
        self.viz_att_maps = False
        self.model_ablate_rff = False  # Apply row wise feed forward network
        ...
        if ln:
            if self.pre_layer_norm:  # Applied to X
                self.ln0 = nn.LayerNorm(dim_Q, eps=1e-12)
            else:  # Applied after MHA and residual
                self.ln0 = nn.LayerNorm(dim_out, eps=1e-12)

            self.ln1 = nn.LayerNorm(dim_out, eps=1e-12)
        else:
            self.ln0 = None
            self.ln1 = None
