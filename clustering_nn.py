...
def train():
    ...
    model = NeuralNetwork(n_features=n_features, n_samples=n_samples, hidden_neuron=32, n_layers=3,
                          k=k)
    ...
    for e in range(epochs): 
        ...
        with torch.no_grad():
            ...
            # on the train
            pred_prob = model(torch.tensor(X_train).float())
            ...
            pred_prob = model(torch.tensor(X_valid).float())
            pred_labels = torch.argmax(pred_prob, axis=1)
            ...
            if best_valid <= valid_completeness:
                ...
                pred_prob = model(torch.tensor(X_test).float())
                pred_labels = torch.argmax(pred_prob, axis=1)
