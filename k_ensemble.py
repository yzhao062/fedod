...
# def train():
    ...
    epochs = 200
    k=10
    predict_test = np.zeros([4, X_test.shape[0]])
    actual_test = np.zeros([4, X_test.shape[0]])
    
    for k_indx, k in enumerate([5,10,15,20]):
        ...
        print(k, 'real', roc_auc_score(y_test, dist_real[:, -1].numpy()), "|", average_precision_score(y_test, dist_real[:, -1].detach().numpy()), "|",precision_n_scores(y_test, dist_real[:, -1].detach().numpy()),"|", 1)
        print(k, 'prediction time', prediction_time)
        print(k, 'cal time', calc_time)
        
        predict_test[k_indx, :] = true_k_values
        actual_test[k_indx, :] = dist_real[:, -1].numpy()
        
#%%
from combo.models.score_comb import average, maximization, median, aom, moa
print("*******************************")
print('real mean', roc_auc_score(y_test, average(actual_test.T)))
print('apprx mean', roc_auc_score(y_test, average(predict_test.T)))
print()

print('real max', roc_auc_score(y_test, maximization(actual_test.T)))
print('apprx max', roc_auc_score(y_test, maximization(predict_test.T)))
print()

print('real aom', roc_auc_score(y_test, aom(actual_test.T, n_buckets=2)))
print('apprx aom', roc_auc_score(y_test, aom(predict_test.T, n_buckets=2)))
print()

print('real moa', roc_auc_score(y_test, moa(actual_test.T, n_buckets=2)))
print('apprx moa', roc_auc_score(y_test, moa(predict_test.T, n_buckets=2)))
