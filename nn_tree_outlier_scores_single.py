...
def collate_batch2(batch, n_estimators, n_trees=10):
    samples = []
    idxs = []
    ...
    # local ground truth
    # 相当于学的是多个iforest的平均->平均的平均
    outputs = np.zeros([n_trees, samples.shape[0]])
...
# def train():
    ...
    outputs = np.zeros([n_trees, X_train.shape[0]]) 
    test_tree = np.zeros([n_trees, X_test.shape[0]]) 
    for i in range(n_trees):    
    ...
    train_loader = torch.utils.data.DataLoader(train_set,
                                               ...
                                               collate_fn=partial(collate_batch2, n_estimators=n_estimators, n_trees=n_trees),
                                               shuffle=True)
